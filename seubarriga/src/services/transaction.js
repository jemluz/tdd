...
module.exports = app => {
  ...
  const save = (transaction) => {
    ...
    if (!transaction.ammount) throw new ValidationError('Valor é um campo obrigatório.')
    if (!transaction.date) throw new ValidationError('Data é um campo obrigatório.')
    if (!transaction.acc_id) throw new ValidationError('Conta é um campo obrigatório.')
